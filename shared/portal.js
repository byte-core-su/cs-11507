...
const teacherTasks = (config.teacherTasks || []).map(([id,label]) => ({id,label}));
const total = teacherTasks.length;
let auth, db, user, profile, taskScores = {}, stop = () => {}, isFinishingInitialSetup = false;
...
function showPortal() {
  const scoreFor = task => taskScores[task.id];
  const hasScore = task => Number.isFinite(Number(scoreFor(task)?.score));
  const completed = teacherTasks.filter(hasScore).length;
...
async function loadStudent(currentUser) {
  ...
  stop();
  stop = onSnapshot(doc(db,'users',user.uid), snap => { taskScores=snap.data()?.teacherProgress?.[app.currentTermId]?.taskScores || {}; showPortal(); });
}
